...
create table if not exists public.admin_sms_logs (
...
create index if not exists admin_sms_logs_created_idx on public.admin_sms_logs (created_at desc);
alter table public.admin_sms_logs enable row level security;
drop policy if exists "admin_sms_logs_admin_read" on public.admin_sms_logs;
create policy "admin_sms_logs_admin_read" on public.admin_sms_logs for select to authenticated using (public.is_app_admin(auth.uid()));
